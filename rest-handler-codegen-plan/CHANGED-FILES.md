...
- **Task 1.8:** `build-tools-internal/src/main/java/org/elasticsearch/gradle/resthandler/RestHandlerGeneratorTask.java` — for each endpoint with serverTransportAction: resolve transport action and listener, emit handler via HandlerCodeEmitter, write JavaFile to output directory; skip endpoints that fail resolution with warn log. **Task 1.11:** clear output directory before generating so removed endpoints do not leave stale generated files.
