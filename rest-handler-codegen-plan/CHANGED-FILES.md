...
- **Task 1.9:** `server/.../rest/action/admin/indices/RestDeleteIndexAction.java`, `RestGetIndicesAction.java` — prepareRequest() refactored to call XxxRequest.fromRestRequest(request) (hand-written handlers since removed in 1.11). cluster.health and cluster.get_settings are unchanged (no fromRestRequest, no spec annotations).
- **Task 1.10:** `server/.../action/ActionModule.java` — registerGeneratedHandlers(registerHandler) via reflection; remove hand-written registration for RestGetIndicesAction, RestDeleteIndexAction, RestPendingClusterTasksAction. RestClusterHealthAction and RestClusterGetSettingsAction remain hand-written and explicitly registered. RestClusterHealthActionTests unchanged (use RestClusterHealthAction.fromRequest as on main).
...
- **Task 1.11:** `server/.../rest/action/admin/indices/RestDeleteIndexAction.java`, `RestGetIndicesAction.java`, `server/.../rest/action/admin/cluster/RestPendingClusterTasksAction.java` — deleted; replaced by generated RestIndicesDeleteAction, RestIndicesGetAction, RestClusterPendingTasksAction in restHandlers source set. RestClusterHealthAction and RestClusterGetSettingsAction are not part of the PoC and remain hand-written.
