...
**Note**: these derived names may NOT match existing handler class names (e.g., the current `RestDeleteIndexAction` would become `RestIndicesDeleteAction`). This is intentional — the generated handlers use this consistent naming scheme. The old hand-written handler is deleted when the generated one replaces it, so name continuity is not required. This convention is the one used by the generator (no special casing).
...
**Handler package** = `org.elasticsearch.rest.action` + everything in the transport action's package after `"action."`.

Example: transport action `org.elasticsearch.action.admin.indices.delete.TransportDeleteIndexAction` has package `org.elasticsearch.action.admin.indices.delete`; the suffix after `"action."` is `admin.indices.delete`, so the generated handler's package is `org.elasticsearch.rest.action.admin.indices.delete`. This keeps generated handlers aligned with the transport layer structure.

**Registry package**: `GeneratedRestHandlerRegistry` lives in the base package `org.elasticsearch.rest.action` (one registry per module, registering all generated handlers from their respective subpackages).
...
**Solution: generated registry class.** In addition to individual handler classes, the generator emits a single `GeneratedRestHandlerRegistry` class that knows how to register all generated handlers. The registry uses `Consumer<RestHandler>` (not `BiConsumer`) to match `ActionModule`'s registration callback:

```java
// Generated by RestHandlerGeneratorTask — do not edit
public class GeneratedRestHandlerRegistry {

    public static void registerHandlers(java.util.function.Consumer<RestHandler> registerHandler) {
        registerHandler.accept(new RestIndicesDeleteAction());
        registerHandler.accept(new RestIndicesGetAction());
...
The registry class lives in the base package `org.elasticsearch.rest.action`.
