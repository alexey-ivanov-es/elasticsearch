...
export function hoistRequestAnnotations (
  ...
  const knownRequestAnnotations = [
    'rest_spec_name', 'behavior', 'class_serializer', 'index_privileges', 'cluster_privileges', 'doc_id', 'availability', 'doc_tag', 'ext_doc_id', 'codegen_exclude', 'server_transport_action', 'server_capabilities', 'server_allow_system_index_access', 'server_can_trip_circuit_breaker'
  ]
  ...
  setTags(jsDocs, request, tags, knownRequestAnnotations, (tags, tag, value) => {
    if (tag.endsWith('_serializer')) {
      ...
      endpoint.docUrlServerless = docUrl[2].replace(/\r/g, '')
      if (endpoint.docUrlServerless === '[use-doc-url]') {
        endpoint.docUrlServerless = endpoint.docUrl
      }
      if (docUrl[3].replace(/\r/g, '') !== '') {
        endpoint.extPreviousVersionDocUrl = docUrl[3].replace(/\r/g, '')
      }
      ...
      if (docUrl[4].replace(/\r/g, '') !== '') {
        endpoint.extDocDescription = docUrl[4].replace(/\r/g, '')
      }
    ...
    } else if (tag === 'server_transport_action') {
      assert(jsDocs, value.trim() !== '', `Request ${request.name.name}'s @server_transport_action cannot be empty`)
      endpoint.serverTransportAction = value.trim()
    } else if (tag === 'server_capabilities') {
    ...
    } else if (tag === 'server_can_trip_circuit_breaker') {
      endpoint.canTripCircuitBreaker = value.trim().toLowerCase() === 'true'
    } else {
